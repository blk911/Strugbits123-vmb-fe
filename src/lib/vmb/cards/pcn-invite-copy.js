...
function parseVisitCount(...sources) {
...
function parseServiceName(...sources) {
    for (const source of sources) {
        if (!source)
            continue;
        ...
        if (service?.[0])
            return service[0].trim();
    }
    return undefined;
}
function haystack(signals) {
    return [
...
function recognitionLine(signals) {
...
export function buildPcnInviteBody(signals) {
...
export function buildPcnInviteSalutation(recipientName) {
    const name = firstName(recipientName);
    return name === "there" ? "Dear friend," : `Dear ${name},`;
}
export function buildPcnInviteCta() {
...
export function pcnSignalsFromText(input) {
    ...
    return {
        ...
        isReferral: hay.includes("referral") ||
