import { buildPersonalInviteCopy, inviteCopyToBody, } from "../cards/personal-invite-copy.js";
export function cardPreviewToQueuedInvitePayload(model, recipientName, actionLabel) {
    const copy = model.inviteCopy ??
...
export function inviteCardToMessage(card) {
    return [
        card.greeting,
...
export function parseQueuedInviteCardPayload(payload) {
    if (!payload || typeof payload.inviteCard !== "object" || !payload.inviteCard) {
        return undefined;
    }
    const raw = payload.inviteCard;
    ...
    return {
        ...
        greeting,
        personalConnection: typeof raw.personalConnection === "string" ? raw.personalConnection : undefined,
...
export function mergeInviteCardIntoDraftPayload(payload, inviteCard) {
    return {
        ...payload,
        inviteCard,
...
export function queueItemPreviewLine(inviteCard, fallbackTitle) {
    if (!inviteCard)
        return fallbackTitle;
    return `${inviteCard.recipientName} — ${inviteCard.actionLabel}`;
}
export function queueItemGreetingPreview(inviteCard) {
    if (!inviteCard?.greeting)
        return undefined;
...
export function resolveInviteCardForDisplay(input) {
    ...
    if (input.fallbackModel) {
        return cardPreviewToQueuedInvitePayload(input.fallbackModel, input.recipientName ?? input.fallbackModel.metadata.recipientName ?? "Client", input.actionLabel ?? "Private Client Invite");
    }
    return undefined;
}
