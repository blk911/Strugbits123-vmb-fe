...
//   name,
//   value,         // 👈 selected value for this field
...
//         {options.map((opt) => (
//           <div key={opt} className={`flex flex-col items-center gap-x-2 ${divClass}`}>
//             <div
//               onClick={() => onChange(opt)} // 👈 send selected option
//               className={`group flex h-4 w-4 items-center justify-center rounded border border-gray-300 cursor-pointer
//                 ${value === opt ? "bg-[#E6E6E6] text-[#FF92A5]" : "bg-[#E6E6E6] text-transparent"} 
//                 ${classes}`}
//             >
//               {value === opt && (
//                 tickIcon ? tickIcon : (
...
export default function CheckboxField({
  ...
  const handleSelect = (opt) => {
    ...
    if (value.includes(opt)) {
      newValue = value.filter((v) => v !== opt); // remove if already selected
    } else {
      newValue = [...value, opt]; // add if not selected
    }
    onChange(newValue);
  };
        ...
        {options.map((opt) => (
          ...
          >
            <div
              ...
              className={`group flex h-4 w-4 items-center justify-center rounded border border-gray-300 cursor-pointer
                ${value.includes(opt) ? "bg-[#E6E6E6] text-[#FF92A5]" : "bg-[#E6E6E6] text-transparent"} 
                ${classes}`}
            >
              {value.includes(opt) &&
                (tickIcon ? (
