...
export default function Button({ text, classes, textclass }) {
  return (
    <button
      ...
      after:content-[''] after:absolute after:h-1 after:w-1 after:bg-[#FF92A5] 
    ...
    >
      ...
      <span className="flex items-center justify-center w-8 h-8 rounded-full bg-[#7a2c3a] relative z-10">
        <svg
          width="13"
          height="9"
          viewBox="0 0 13 9"
    ...
    // <button
    //   class="px-8 z-30 py-4 bg-rose-400 rounded-md relative font-semibold
    //      overflow-hidden transition-all duration-700 text-2xl
    //      [text-shadow:3px_5px_2px_#be123c;] hover:[text-shadow:2px_2px_2px_#fda4af]
    //      after:content-[''] after:absolute after:h-1 after:w-1 after:bg-rose-800
    //      after:left-1/2 after:bottom-0 after:-translate-x-1/2 after:translate-y-full
    //      after:rounded-md after:z-0 after:transition-all after:duration-700
    //      hover:after:scale-[300]"
    // >
    //   <span class="relative z-10 text-white">Hover Me</span>
    // </button>
  );
}
