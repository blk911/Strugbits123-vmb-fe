...
export default function Requests() {
  ...
  const totalPagesByTab = {
    All: totalPages,
    Confirmed: Math.ceil(confirmedGifts.length / PAGE_SIZE),
    Pending: Math.ceil(pendingGifts.length / PAGE_SIZE),
    Declined: Math.ceil(declinedGifts.length / PAGE_SIZE),
  };
  const cleanDataForTable = (data) =>
    data.map(({ _modalData, ...rest }) => rest);

  const tabs = {
    All: cleanDataForTable(allGifts),
    Confirmed: cleanDataForTable(confirmedGifts),
    Pending: cleanDataForTable(pendingGifts),
    Declined: cleanDataForTable(declinedGifts),
  };

  const originalRows = {
    All: allGifts,
    Confirmed: confirmedGifts,
    Pending: pendingGifts,
    Declined: declinedGifts,
  };
  ...
  const handleRowClick = {
    ...
    Declined: (cleanRow) => openWithData(cleanRow, "Declined"),
  };

  const openWithData = (cleanRow, tab) => {
    const row = originalRows[tab].find((r) => r.id === cleanRow.id);
    if (row?._modalData) {
      // openModal("giftRequestHistory", row._modalData);
      openModal("giftRequestHistory", {
        timelineItems: [
          {
            icon: <FaPaperPlane className="w-4 h-4" />,
  ...
  const handlePageChange = (tab, page) => {
    ...
    if (tab === "Declined") setDeclinedPage(page);
  };
  ...
  return (
    <div className="w-full flex flex-col gap-y-[31px] py-6 bg-[#EFEFEF]">
      <TabbedTable
        tabs={tabs}
        tabOrder={["All", "Confirmed", "Pending", "Declined"]}
        defaultTab="All"
        cellRenderers={CellRenderers}
        tabLabelMap={{
          All: "All",
