...
function SalonCard({ img, label, title, listItems, btnText, classes = "" }) {
    ...
    return (
        <div className={`flex max-xl:gap-x-[20px] lg:gap-x-[70px] gap-y-[20px] items-center ${classes}`}>
            <div className='max-md:w-full lg:w-[500px] h-[411px] border-1 border-[#581838] rounded-[10px] overflow-hidden'
                style={shadow}
            >
            ...
            <div className='max-md:w-full lg:w-[483px] flex flex-col gap-y-[10px]'>
                ...
                <Button text={btnText} classes={'mt-[30px] w-max'} />
            </div>
        </div>
    )
}
...
function SalonChoice() {
    ...
    return (
        <div className='w-full flex flex-col justify-center items-center py-[150px] gap-y-[150px] sm:px-[50px] px-[10px] '>
            <SalonCard
                img={pic1}
                label="Salon Owners"
