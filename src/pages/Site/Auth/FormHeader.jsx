...
import { setAuthMode, setNext } from '../../../store/features/authSlice';
...
function FormHeader({mode}) {
    ...
    return (
        <>
            ...
            {/* Switch buttons */}
            <div className="w-max bg-[#FF92A566] rounded-[50px] flex gap-x-[10px] p-[7px]">
                <Button
                    text="Sign In"
                    textClasses={"!text-[#581838]"}
                    onClick={() => {
                        dispatch(setAuthMode("login")); dispatch(setNext())
                    }}
                    ...
                    classes={`!rounded-[50px] ${mode === "login" ? "!bg-white" : "!bg-transparent"}`}
                />
                <Button
                    text="Sign Up"
                    textClasses={"!text-[#581838]"}
                    ...
                    onClick={() => {
                        dispatch(setAuthMode("signup")); dispatch(setNext())
                    }}
                    classes={`!rounded-[50px] ${mode === "signup" ? "!bg-white" : "!bg-transparent"}`}
                />
            </div>
